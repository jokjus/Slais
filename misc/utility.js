...
let utils = {
	...
	getRandomInts: function(min, max, amount) {
		let randoms = []
		for (let i = 0; i < amount; i++) {
			randoms.push(Math.floor(Math.random() * Math.floor(max-min)) + min)
		}
		return randoms
	},
	getRandomSpacedInts: function(min, max, count, spacing) {
		...
		for (var i = 0; i<count; i++) {
			...
			arr[i]       = parseInt(((i==0)? min+temp : arr[i-1] + temp + spacing));
			available   -= temp;
		}
		return arr;
	},
	...
	addPointToCurve: function(path, offset) {
